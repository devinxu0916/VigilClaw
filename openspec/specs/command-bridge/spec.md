...
CommandBridge 是宿主进程内的 TCP HTTP 服务，为容器内 Agent 提供特权系统操作通道。每个 Agent 任务拥有独立的 CommandBridge 实例，监听随机端口，将容器的系统命令请求委托给宿主进程的核心对象（DB、SkillRegistry、TaskScheduler、SessionManager）。
